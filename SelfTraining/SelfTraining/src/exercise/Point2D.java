...
public class Point2D implements Comparable<Point2D> {
   ...
   public final Comparator<Point2D> GREATER_DISTANCE = new GreaterDistanceToOrder();
    ...
    // compare points according to their distance to this point
    private class GreaterDistanceToOrder implements Comparator<Point2D> {
        public int compare(Point2D p, Point2D q) {
            double dist1 = distanceSquaredTo(p);
            double dist2 = distanceSquaredTo(q);
            if      (dist1 < dist2) return +1;
    ...
    public static void main(String[] args) {
        ...
        Arrays.sort(points, p.GREATER_DISTANCE);
        
        for (int i = 0; i < N; i++) {
            StdDraw.setPenRadius(.005);	
            points[i].draw();
            StdDraw.show(100);
           if(p.distanceTo(points[i]) <= 23 ){ 
        	 cluster1.add(points[i]);
             StdDraw.setPenRadius(.002);	
         	p.drawTo(points[i]);
            StdDraw.show(300);
            System.out.println(points[i]+ "  " + p.distanceTo(points[i]));
           }
    //       
        }
...
/*        for (int i = 0; i < cluster1.size(); i++)
        ...
        if(i == (cluster1.size()-1))
        	j=0;
        Point2D p1 =	cluster1.get(i);
        Point2D p2 =	cluster1.get(j);
        
    	p1.drawTo(p2);
        StdDraw.show(100);
        }
    	cluster1.get(0).drawTo(cluster1.get((cluster1.size()-1)));
        StdDraw.show(100);
 /*       for(int i=0 ; i< N; i++)
        {
        	for(int j=0;j<N;j++){
